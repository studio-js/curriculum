      ...
      nodes: [
        {
          ...
          lessons: [
            { title: '결측값 유형(MCAR·MAR·MNAR)과 처리 전략', hours: 6, summary: '결측값이 발생하는 세 가지 메커니즘(MCAR·MAR·MNAR)을 구분하고, 각 유형에 적합한 제거·대체·다중대체 전략을 선택하는 기준을 학습한다.', objectives: ['MCAR·MAR·MNAR를 데이터 패턴으로 진단한다', '평균·중앙값·최빈값 대체와 KNN 대체를 상황에 맞게 적용한다', '결측 비율에 따른 처리 전략 의사결정 트리를 작성한다'] },
            { title: '이상값 탐지: IQR 방법과 Z-score', hours: 5, summary: 'IQR 기반 펜스 방법과 Z-score를 사용해 이상값을 탐지하고, 비즈니스 맥락에 따라 제거·캡핑·변환 중 적절한 처리 방식을 선택하는 방법을 학습한다.', objectives: ['IQR 방법으로 이상값 경계를 계산하고 시각화한다', 'Z-score 기준을 설정해 정규분포 기반 이상값을 탐지한다', '이상값 처리 전후 분포 변화를 히스토그램으로 비교한다'] },
            { title: '중복 제거와 데이터 타입 변환', hours: 4, summary: '중복 행을 탐지·제거하는 기준을 설계하고, 문자열로 저장된 숫자·날짜 컬럼을 올바른 dtype으로 변환해 분석 정확도를 높이는 방법을 실습한다.', objectives: ['duplicated()와 drop_duplicates()로 중복을 탐지하고 제거한다', 'astype·pd.to_numeric·pd.to_datetime으로 타입을 변환한다', '타입 불일치로 발생하는 집계 오류를 진단하고 수정한다'] },
            { title: '문자열 전처리: 정규표현식·str accessor', hours: 5, summary: 'Pandas str accessor와 정규표현식을 결합해 비정형 텍스트 컬럼에서 패턴을 추출·치환하고, 표준화된 형식으로 정제하는 파이프라인을 구현한다.', objectives: ['str.contains·str.extract로 패턴을 탐지하고 추출한다', '정규표현식으로 전화번호·이메일 등 형식을 표준화한다', 'str.replace와 str.strip으로 불필요한 공백·특수문자를 제거한다'] },
            { title: 'groupby 심화: transform·apply·agg', hours: 7, summary: 'groupby와 transform·apply·agg를 조합해 그룹별 집계·파생 변수 생성·복잡한 커스텀 함수 적용까지 다양한 집계 패턴을 실습한다.', objectives: ['agg로 그룹별 다중 통계량을 한 번에 계산한다', 'transform으로 그룹 평균을 원본 인덱스에 맞게 반환한다', 'apply로 그룹 단위 커스텀 로직을 DataFrame에 적용한다'] },
            { title: 'pivot_table과 crosstab 분석', hours: 6, summary: 'pivot_table로 다차원 집계 테이블을 생성하고, crosstab으로 범주형 변수 간 빈도·비율을 교차 분석하는 방법을 학습한다.', objectives: ['pivot_table의 values·index·columns·aggfunc 파라미터를 설정한다', 'crosstab으로 두 범주형 변수의 교차 빈도를 계산한다', '피벗 결과를 히트맵으로 시각화해 인사이트를 도출한다'] },
            { title: '멀티인덱스와 계층형 데이터 핸들링', hours: 5, summary: '다단계 groupby 결과로 생성되는 멀티인덱스 DataFrame을 탐색·조작하고, stack·unstack·reset_index로 원하는 형태로 변환하는 방법을 실습한다.', objectives: ['멀티인덱스 DataFrame에서 xs와 loc로 특정 레벨을 선택한다', 'stack·unstack으로 행·열 방향을 전환한다', 'reset_index로 멀티인덱스를 일반 컬럼으로 평탄화한다'] },
            { title: '데이터 병합: merge·join·concat 전략', hours: 7, summary: 'merge·join·concat의 동작 방식과 키 설정·방향성 차이를 이해하고, 실무 데이터 통합 시나리오에서 최적의 병합 전략을 선택하는 방법을 학습한다.', objectives: ['inner·left·right·outer 조인의 결과 차이를 비교한다', 'merge의 on·left_on·right_on 파라미터로 복합 키를 설정한다', 'concat의 axis·ignore_index 옵션으로 데이터를 수직·수평으로 연결한다'] },
            { title: '날짜·시간 데이터 처리 (DatetimeIndex)', hours: 6, summary: 'DatetimeIndex를 활용해 날짜 기반 슬라이싱·리샘플링·시간 이동을 수행하고, 시계열 분석의 기반이 되는 날짜 파생 변수를 생성하는 방법을 실습한다.', objectives: ['pd.to_datetime으로 문자열을 DatetimeIndex로 변환한다', 'resample로 일별·주별·월별 집계를 수행한다', 'dt accessor로 연·월·요일·시간 파생 변수를 생성한다'] },
            { title: '전처리 전후 분포 비교 시각화', hours: 6, summary: '결측값·이상값·정규화 처리 전후의 분포 변화를 히스토그램·박스플롯·QQ-플롯으로 비교하고, 전처리 품질을 시각적으로 검증하는 방법을 학습한다.', objectives: ['전처리 전후 히스토그램을 나란히 배치해 분포 변화를 비교한다', '박스플롯으로 이상값 처리 효과를 시각화한다', 'QQ-플롯으로 정규성 근사 여부를 시각적으로 확인한다'] },
            { title: '전처리 파이프라인 자동화 실습', hours: 6, summary: 'sklearn Pipeline과 ColumnTransformer를 활용해 결측값 처리·인코딩·스케일링을 하나의 재사용 가능한 전처리 파이프라인으로 구성하는 방법을 실습한다.', objectives: ['sklearn Pipeline으로 전처리 단계를 순서대로 체이닝한다', 'ColumnTransformer로 수치형·범주형 컬럼에 다른 변환을 적용한다', '파이프라인을 joblib으로 저장하고 새 데이터에 재적용한다'] },
          ],
